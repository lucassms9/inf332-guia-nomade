const sugestao = {
  trabalho: {
    nomadedigital: {
      lugares: ["10020", "11128"],
      acomodacoes: ["19010", "19013"],
      happyHour: ["13003"],
      eventos: ["14001", "14002"],
      profissoes: ["Profissionais de TI", "Designers", "Redatores"],
  ...
  ferias: {
    turismocultural: {
      lugares: ["10027", "10004"],
      acomodacoes: ["19014"],
      happyHour: ["13027", "13383"],
      eventos: ["Festivais culturais", "Exposições de arte", "Palestras"],
      profissoes: ["Historiadores", "Professores", "Guias turísticos"],
    ...
    aventuraesportes: {
      lugares: ["10001", "18000"],
      acomodacoes: ["19016"],
      happyHour: ["13332", "13338"],
      eventos: ["14007"],
      profissoes: ["Instrutores de esportes", "Treinadores físicos"],
      faixaEtaria: { inicial: 18, final: 45 },
      faixaValor: { inicial: 60, final: 200 },
    },
    ecoturismonatureza: {
      lugares: ["16034"],
      acomodacoes: ["19016", "16008"],
      happyHour: ["16036", "13377"],
      eventos: ["10056", "16019"],
      profissoes: ["Biólogos", "Ecologistas", "Naturalistas"],
      faixaEtaria: { inicial: 25, final: 55 },
      faixaValor: { inicial: 70, final: 180 },
    },
    viagemdeluxo: {
      lugares: ["10028", "10037", "10043", "13387"],
      acomodacoes: ["19014", "19018"],
      eventos: ["14006", "14005"],
      happyHour: ["13148", "13236"],
      profissoes: ["Empresários", "Executivos", "Celebridades"],
      faixaEtaria: { inicial: 30, final: 65 },
      faixaValor: { inicial: 200, final: 500 },
    },
    mochilao: {
      lugares: ["10027", "10047", "10069"],
      acomodacoes: ["19013"],
      eventos: ["14004", "14005"],
      happyHour: ["13002", "13031", "13037"],
      profissoes: ["Estudantes", "Artistas", "Freelancers"],
...
searchPlaceFourSquareByLatLongAndCategories = async (ll, categories) => {
  ...
  console.log(
    "await searchPlaceFourSquareByLatLongAndCategories:",
    ll,
    categories
  );
  ...
  return (
    (await Promise.all(
      data.results.map(async (d, index) => {
        ...
        let photos =
          placePhotos?.data?.map((p) => `${p.prefix}original${p.suffix}`) || [];
        return {
          id: fsq_id,
...
exports.lugares = async (req, res) => {
  let { modo, estilo } = req.params;
  let { ll, tag } = req.query;
  ...
  try {
    ...
    if (!placesWithPhotos || !placesWithPhotos.length) {
      return res
        .status(404)
        .json({ modo, estilo, ll, message: "não encontrado." });
    }
...
exports.recomendar = (req, res) => {
  ...
  if (Number.isNaN(idade) || Number.isNaN(valorMedioPorDia)) {
    return res
      .status(400)
      .json({ mensagem: "idade e valorMedioPorDia devem ser números" });
  }
  ...
  for (const modo in sugestao) {
    for (const estilo in sugestao[modo]) {
      ...
      if (
        ...
        (idade >= info.faixaEtaria.inicial &&
          idade <= info.faixaEtaria.final) ||
        (valorMedioPorDia >= info.faixaValor.inicial &&
